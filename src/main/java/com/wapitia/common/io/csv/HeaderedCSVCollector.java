...
public class HeaderedCSVCollector implements CSVCellCollector {
    ...
    static enum HDR_ROW_STATE {
        BEFORE_HDR_ROW, ON_HDR_ROW, AFTER_HDR_ROW
    };
    ...
    /**
     *
     * by default, the first non-empty cell is the start corner of the header.
     */
    protected boolean isHeaderStart(int column, long row, String cellContents) {

        final boolean isNotEmpty = (cellContents != null)
            && (cellContents.trim().length() > 0);
        return isNotEmpty;
    }
    ...
    public static HeaderedSpreadsheet spreadsheeetOf(final CharSequence s) {

        final HeaderedSpreadsheet result = new HeaderedCSVCollector()
            .getHeaderedSpreadsheet(s);
        return result;
    }
